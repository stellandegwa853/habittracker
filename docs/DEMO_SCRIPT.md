...
| 5 | Show `/dashboard`. | The dashboard summarizes today's habits, completed habits, streaks, and progress. | Demo, UI, functional requirements | Existing: `presentation_screenshots/01_dashboard.png`. |
...
| 13 | Open Calendar. | Calendar shows completion history based on backend habit completion dates. | Demo, history | Existing: `presentation_screenshots/03_calendar.png`. |
| 14 | Open Statistics. | Statistics use saved habit and completion data to calculate progress. | Demo, progress | Existing: `presentation_screenshots/04_statistics.png`. |
...
| 18 | Show GitHub repository. | Explain version control, commits, branches, and contributors. | Version control/teamwork | Existing: `presentation_screenshots/05_git_history.png`; add contributors/branches screenshots if possible. |
...
| 20 | Show terminal checks. | Run or show evidence for lint, build, Django check, and Django tests. | Testing | Existing: `presentation_screenshots/08_build_quality_testing.png`; update if needed. |
...
6. "Testing is still basic, but the current Django tests, Django system check, frontend lint, and frontend build all pass."
